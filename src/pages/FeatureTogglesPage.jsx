...
export default function FeatureTogglesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await client.get('/admin/feature-toggles');
  ...
  const toggleFeature = async (feature, isEnabled) => {
    setSaving(feature);
    try {
      const { data } = await client.post('/admin/feature-toggles', { feature, isEnabled: !isEnabled });
  ...
  const addFeature = async (e) => {
    ...
    try {
      await client.post('/admin/feature-toggles', { feature: newFeature.trim(), isEnabled: false, description: newDescription.trim() });
      const { data } = await client.get('/admin/feature-toggles');
      setToggles(data.toggles || []);
      setNewFeature('');
      setNewDescription('');
...
const styles = {
  ...
  addBtn: { background: '#FFD700', color: '#FFF', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer', fontSize: 14 },
  ...
  toggleOn: { background: '#FFD700', color: '#FFF' },
